...
async function persistLegacyMappedSettings(
  input: z.infer<typeof legacyUpdateSettingsSchema>
) {
  ...
  if (input.supportEmail !== undefined)
    updates.push({ key: "store.email", value: input.supportEmail });
  if (input.footerAddress !== undefined)
    updates.push({ key: "store.address", value: input.footerAddress });
  if (input.storeCurrency !== undefined)
    updates.push({ key: "store.currency", value: input.storeCurrency });
  if (input.deliveryText !== undefined)
    updates.push({ key: "store.deliveryText", value: input.deliveryText });
  if (input.paymentMethodsText !== undefined)
    updates.push({
      key: "store.paymentMethodsText",
      value: input.paymentMethodsText,
    });
  if (input.promoActive !== undefined)
    updates.push({ key: "promo.active", value: String(input.promoActive) });
  if (input.promoKicker !== undefined)
    updates.push({ key: "promo.kicker", value: input.promoKicker });
  if (input.promoTitle !== undefined)
    updates.push({ key: "promo.title", value: input.promoTitle });
  if (input.promoSubtitle !== undefined)
    updates.push({ key: "promo.subtitle", value: input.promoSubtitle });
  if (input.promoLinkLabel !== undefined)
    updates.push({ key: "promo.linkLabel", value: input.promoLinkLabel });
  ...
  app.put(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const before =
          (await db.listShippingZonesWithRates()).find(
            zone => zone.id === id
          ) || null;
  ...
  app.put(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const before =
          allZones.flatMap(zone => zone.rates).find(rate => rate.id === id) ||
          null;
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const explicitPassword =
          ...
          parsed.password.trim().length > 0
            ? parsed.password.trim()
            : null;
        if (explicitPassword) {
          const policy =
            adminSecurity.validatePasswordStrength(explicitPassword);
          if (!policy.ok) {
            sendError(
              res,
              400,
              policy.message || "Password does not meet security policy."
            );
            return;
          }
        }
        ...
        const tempPassword =
          explicitPassword || `SBP-${crypto.randomBytes(5).toString("hex")}`;
  ...
  app.put(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        await db.updateAdminUserRole(
          id,
          mapRoleToAdminRole(nextRole),
          nextActive
        );
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const tempPassword =
          parsed.newPassword || `SBP-${crypto.randomBytes(5).toString("hex")}`;
        if (parsed.newPassword) {
          const policy = adminSecurity.validatePasswordStrength(
            parsed.newPassword
          );
          if (!policy.ok) {
            sendError(
