...
function sanitizeCssVariableName(input: string) {
  return input.replace(/[^a-zA-Z0-9_-]/g, "").slice(0, 64);
}

function sanitizeCssColorValue(input: string) {
  const value = input.trim();
  if (!value) return null;

  // Allow common safe color formats only.
...
${colorConfig
  .map(([key, itemConfig]) => {
    ...
    const color =
      ...
      itemConfig.color;
    const safeColor = typeof color === "string" ? sanitizeCssColorValue(color) : null;
